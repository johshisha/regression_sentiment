...
parser.add_argument('--mean', '-m', default='resource/alex_mean.npy',
...
if args.write_f:
    name = 'resource/fc7_features/fc7_%s'%args.test.split('/')[-1]
    print('Write features > %s'%name)
    import csv
    f = open(name,'wb')
...
try:
    test_data, test_label = load_image_from_list.for_regression(args.test, args.root)
    orig = True
except:
...
def feed_data():
    ...
    test_y_batch = np.ndarray((test_batchsize,), dtype=np.float32)

    test_batch_pool = [None] * test_batchsize
    test_pathes = [None] * test_batchsize
    

    pool = multiprocessing.Pool(args.loaderjob)
    data_q.put("test")
    for path,label in zip(test_data,test_label):
        test_batch_pool[i] = pool.apply_async(read_image,(path,True,False))
        test_y_batch[i] = label
        test_pathes[i] = path
        i += 1
        ...
        if i == test_batchsize:
            ...
            if None in test_pathes:
                print('include none')
            data_q.put((test_pathes, test_x_batch.copy(), test_y_batch.copy()))
            i = 0
            test_x_batch = np.ndarray((test_batchsize, 3, model.insize, model.insize), dtype=np.float32)
            test_y_batch = np.ndarray((test_batchsize,), dtype=np.float32)
    ...
    if i != 0:
        for k, x in enumerate(test_batch_pool[:i]):
            test_x_batch[k] = x.get()
        data_q.put((test_pathes, test_x_batch.copy()[:i], test_y_batch.copy()[:i]))
        i = 0
...
def log_result():
    ...
    while True:
        ...
        if args.print_f:
            if orig:
                print('"%s": %f (orig: %f)'%(path,float(result),float(label)))
            else:
                print('"%s": %f'%(path,float(result)))
        """    
        if args.write_f:
            ...
            sys.stderr.write(
                ...
                .format(samples, datetime.timedelta(seconds=duration), throughput))
                
            #writer.writerow((path,float(result),float(label)))
            for i in range(len(label)):
                data = list(result[i])
                data.insert(0, float(label[i]))
                data.insert(0, path[i])
...
def test_net():
    while True:
        ...
        t = chainer.Variable(xp.asarray(label).reshape(len(label),1), volatile=volatile)

        model(x,t)
        res_q.put([path,model.h7.data,label])
        del x, t
