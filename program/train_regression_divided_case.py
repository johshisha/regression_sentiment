...
import compute_mean, load_image_from_list
...
parser.add_argument('image_file_dir', help='Path to data image-label list file directory')
...
parser.add_argument('--root', '-r', default='resource/images',
...
parser.add_argument('--initmodel', default='resource/alex_regression.model',
...
def remove_broken_image(datas, labels, root):
    count = 0
    for i in range(len(datas)):
        filepath = os.path.join(root, datas[i - count])
        try:
            im = Image.open(filepath).resize((256,256))
            image = np.asarray(im).transpose(2, 0, 1)
        except:
...
out_path = 'resource/cv_regression_divided_case_model/remove_%d'%test_k
...
out_state = os.path.join(out_path, 'state')
...
for i in range(1, k+1):
    if i != test_k:
        ...
        data_list, label_list = load_image_from_list.for_regression(path, args.root)
...
data_test, label_test = load_image_from_list.for_regression(path, args.root)

print('train:', len(data_train), ' test:', len(data_test))

# Prepare model
import alex_model_for_regression_divided_case
model = alex_model_for_regression_divided_case.Alex()
...
def feed_data():
    ...
    for epoch in six.moves.range(1, 1 + args.epoch):
        print('\nepoch', epoch, 'removed', test_k, file=sys.stderr)
        ...
        for idx in perm:
            ...
            if count % denominator == 0:
                data_q.put('val')
                j = 0
                for path, label in zip(data_test,label_test):
        ...
        if epoch % 10 == 0 and not epoch == 0:
            ...
            serializers.save_npz(out_state, optimizer)
...
def log_result():
    ...
    while True:
        ...
        if result == 'end':
            ...
            print(file=sys.stderr)
            train = False
            val_count = val_loss = 0 #val_accuracy = 0
            val_begin_at = time.time()
            continue

        loss = result #, accuracy = result
...
def train_loop():
    ...
    while True:
        ...
        t = chainer.Variable(xp.asarray(inp[1]).reshape(len(inp[1]),1) , volatile=volatile)
        if model.train:
        ...
        #print('debag(model.loss.data):',float(model.loss.data))
        #if model.loss.data != model.loss.data:
        #    print('nan')            
        #    sys.exit(-1)
...
# Save final model
serializers.save_npz(out_model+'_last', model)
serializers.save_npz(out_state+'_last', optimizer)
