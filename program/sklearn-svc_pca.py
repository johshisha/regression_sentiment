...
out_path = 'resource/cv_svc_pca_model/remove_%d'%test_k
...
data_list, label_list, path_list = util.load_fc7_features_c(path)
...
data_test, label_test, path_test = util.load_fc7_features_c(path)
...
if os.path.isfile(out_model) and not args.train_f:
    ...
    tuned_parameters = [{'C': [10**i for i in range(1,4)]}]
    gscv = GridSearchCV(svm.LinearSVC(), tuned_parameters, cv=5)

    #gscv = svm.LinearSVC(C=1)
...
if args.write_f:
    for p,r,l in zip(path,result,label):
